...
<table class="table">
    <tr>
        <td><strong><?= Yii::t('user', 'Registrado el') ?>:</strong></td>
    ...
    <tr>
        <td><strong><?= Yii::t('user', 'Estado') ?>:</strong></td>
        <?php if ($user->isBlocked): ?>
            <td class="text-danger">
                <?= Yii::t('user', 'Bloqueado el {0, date, MMMM dd, YYYY HH:mm}', [$user->blocked_at]) ?>
            </td>
        <?php else: ?>
            <td class="text-success"><?= Yii::t('user', 'Activo') ?></td>
        <?php endif ?>
    </tr>
</table>
